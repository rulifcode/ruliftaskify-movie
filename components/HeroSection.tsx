...
import { Movie } from "@/types/movie"; // ← import dari types, hapus definisi lokal

// Hapus ini semua (type lokal bentrok dengan types/movie.ts):
// type Movie = {
//   id: number;
//   title: string;
//   ...
        ...
        <div style={{ opacity: fade ? 1 : 0, transition: "opacity 0.6s ease" }}>
          ...
          {currentHero && (
            <p className="text-sm text-white/40 font-medium tracking-wide">
              Featured: <span className="text-white/70">{getTitle(currentHero)}</span> {/* ← pakai getTitle */}
